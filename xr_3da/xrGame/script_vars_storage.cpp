...
int  CScriptVarsTable::load(IReader  &memory_stream)
{
	...
	{		
		...
		Msg ("3SCRIPT_VARS", "#DBG: CScriptVarsTable::load n_var = %3d type = 0x%08x   size = %5d  name =  %s.%s   ", n_var, sv.type, sv.size, name(), *var_name);
		if (LUA_TTABLE == sv.eff_type())
		{			
...
void CScriptVarsTable::get(lua_State *L, LPCSTR k, bool unpack)
{
	...
	try
	{
		...
		{			
			...
			Msg(msg); // WARN: имплементаци€ этой функции должна быть из последней ревизии с 4-кебибайтными буферами
