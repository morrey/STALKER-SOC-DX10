...
//#include <D3Dcompiler.h> // todo: use D3DDisassemble
	...
	void*							_ppConstantTable)
{
   ...
   if( o.dx10_msaa )
   {
	...
	if (SUCCEEDED(_result) && o.disasm)
	{
		...
		//ID3DBlob*		disasm	= 0; // originally used
        ID3DXBuffer*		disasm = 0;
        //D3DDisassemble(code->GetBufferPointer(), code->GetBufferSize(), 0, 0, &disasm); // to be used
		//D3DX10DisassembleShader(code->GetBufferPointer(), code->GetBufferSize(), FALSE, 0, &disasm ); // originally used
