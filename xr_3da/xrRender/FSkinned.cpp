...
struct	vertHW_1W
{
	...
	void get_pos_bones( Fvector& p, IKinematics* Parent ) const
	{
...
#if !defined(USE_DX10) && !defined(USE_OGL)	//	Don't use hardware buffers in DX10 since we can't read them
	switch	(RenderMode)
	{
	case RM_SKINNING_SOFT:
#endif	//	!USE_DX10 && !USE_OGL
...
template<typename T>
IC void get_pos_bones(const T& v, Fvector& p, IKinematics* Parent )
{
